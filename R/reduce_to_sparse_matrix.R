#' reduce_to_sparse_matrix
#'
#' Convert a sparse tensor X into the sparse matrix format.
#'
#' @param X X has attributes $i, $j, $v, $nrow, $ncol, $dimnames
#' @return Output X has the same attributes as the input X, But $i and $j are forced to be in vector instead of matrix, and $nrow and $ncol are forced to be scalars instead of vectors.
#' @export
