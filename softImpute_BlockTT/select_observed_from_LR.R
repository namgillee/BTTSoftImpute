select_observed_from_LR <- function(x, numobs = NULL, rate = 1.0)
{
  ...
  # Inputs:
  #   data matrix x     - a LR (Low-rank (SVD)) format with attributes x$u, x$d, x$v, 
  #                       and x$u is a matrix, x$v is a blockTT format.
  #
  #   numobs, rate      - first, numobs is used if it is not NULL. 
