bttdSsvdImpute_ALS_v01 <- function(Y, lambda, ttrank_max, tol_x, 
                               maxit, verbose, X0 = NULL, final.svd=FALSE) 
{
  ...
  #
  # *****************************************************************************
  # * In the version 'als_v01', the matrix Afill = {Ares + udv'} are updated at *
  # * every iteration of updating u and v, i.e.,                                *
  # * U -> (update) -> V1 -> V2 -> ... -> V1 -> (update)                        *
  # *****************************************************************************
  #
  ...
  while ((ratio > tol_x) & (iter < maxit)) {
    ...
    #----
    # Assume that Afill = {Ares + u_old * d_old * v_old'}
    # Note that, in this process, u,d,v are fixed and 
    # updated after an iteration of updating (u,d,v).
    #----
    
    # Update u
    # u <- Afill %*% v
    u <- sp_times_btt(Ares, v) + u %*% diag(d) 
    ...
    # Below, be careful about u_old, v_old, d_old, 
    # regarding lrRatio calculation.
    # Ares = A_omega - X_omega
    Ares = sp_minus_lr_btt(Y, list(u=u,d=d,v=v))
    u_old = u  #<-- u and d  are  changed after the last iteration of updating V1,...,Vn,V{n-1},...,V1
    v_old = v
    d_old = d 
    ...
    for (i in 1:length(course)) {
      ...
      ##Do not update Ares every time. 
      ## Keep using u_old, d_old, v_old
      ...
      VAU = sp_times_btt(Ares, vFrame) #Ares * V_{neq}
      VAU = t(VAU) %*% u    #(V_{neq}' * Ares') * u    ### <-- Note that u and d are not changed during iterations on cores of V,
      VAU = VAU + bttDot(vFrame, v_old) %*% diag(d)    ### <-- i.e., u_old = u, d_old = d.
